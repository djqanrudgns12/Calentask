...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-card/95 backdrop-blur-md px-3 py-2 rounded-xl border border-border shadow-lg text-[12px] font-bold z-50">
    ...
    return (
      <div className="bg-card/95 backdrop-blur-md px-3 py-2 rounded-xl border border-border shadow-lg text-[12px] font-bold">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-muted-foreground ml-1">{d.hours.toFixed(1)}h ({d.percentage}%)</span>
        </div>
      </div>
    )
  }
  ...
  if (isLoading) {
    return (
      <div className="space-y-6 animate-pulse">
        ...
        <div className="h-[200px] bg-muted rounded-3xl" />
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {donutData.length > 0 && (
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-[15px] font-extrabold text-foreground tracking-tight">카테고리 점유율</h3>
          ...
          <div className="flex flex-col md:flex-row gap-6">
            ...
            <div className="w-[160px] h-[160px] relative shrink-0 mx-auto md:mx-0">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                ...
                <span className="text-[15px] font-black text-foreground">{Math.round(categoryBreakdown.totalMinutes / 60)}h</span>
              </div>
            </div>
            ...
            <div className="flex-1 space-y-2.5">
              {donutData.slice(0, 6).map((d, i) => (
                <button
                  ...
                  className="w-full flex items-center justify-between py-1.5 px-2 rounded-lg hover:bg-muted transition-colors group"
                >
                  <div className="flex items-center gap-2">
                    <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: d.color }} />
                    <span className="text-[13px] font-bold text-foreground group-hover:text-indigo-600 transition-colors">{d.name}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-[13px] font-bold text-foreground">{d.hours}h</span>
                    <span className="text-[11px] font-bold text-muted-foreground">{d.percentage}%</span>
                  </div>
                </button>
              ))}
            </div>
      ...
      {categoryBreakdown.items.length > 0 && (
        ...
        >
          <h3 className="text-[17px] font-extrabold text-foreground mb-4 tracking-tight">카테고리별 상세</h3>
            ...
            {categoryBreakdown.items.slice(0, 6).map((item, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <div>
                    <div className="text-[32px] font-black text-foreground tracking-tighter leading-none">
                      {hours}<span className="text-[16px] text-muted-foreground font-bold ml-0.5">h</span>
                    </div>
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className="text-[12px] font-bold text-muted-foreground">{item.count}건</span>
                    </div>
                  </div>
                </motion.div>
              )
