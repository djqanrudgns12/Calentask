...
export default function TemplateCenterTab() {
  ...
  const getMedalIcon = (idx: number) => {
    ...
    if (idx === 1) return <Medal className="w-4 h-4 text-muted-foreground" />
  ...
  const renderTimePrimary = (mins: number) => {
    if (mins === 0) {
      return (
        <>
          <span className="text-[24px] font-black text-foreground tracking-tighter">0</span>
          <span className="text-[13px] font-bold text-muted-foreground ml-1">분</span>
        </>
      )
    }
    if (mins >= 60) {
      ...
      return (
        <>
          <span className="text-[24px] font-black text-foreground tracking-tighter">{h}</span>
          <span className="text-[13px] font-bold text-muted-foreground ml-1">시간</span>
          {m > 0 && (
            <>
              <span className="text-[24px] font-black text-foreground tracking-tighter ml-1.5">{m}</span>
              <span className="text-[13px] font-bold text-muted-foreground ml-1">분</span>
            </>
          )}
        </>
    ...
    return (
      <>
        <span className="text-[24px] font-black text-foreground tracking-tighter">{mins}</span>
        <span className="text-[13px] font-bold text-muted-foreground ml-1">분</span>
      </>
    )
  }
    ...
    return (
      <div className="space-y-6 animate-pulse mt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[1,2,3].map(i => <div key={i} className="h-64 bg-muted rounded-3xl" />)}
        </div>
      </div>
    )
  }
    ...
    <div className="space-y-6 mt-2">
      ...
      {ranked.length > 0 && totalMonthHours > 0 && (
        <motion.div
          ...
          className="bg-card rounded-[20px] p-5 shadow-sm border border-border"
        >
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-[13px] font-bold text-muted-foreground tracking-wider">{currentLabel} 템플릿 랭킹</h3>
            <span className="text-[13px] font-bold text-foreground">총 {totalMonthHours.toFixed(1)}시간</span>
          </div>
          ...
          <div className="flex items-center gap-4 flex-wrap">
            {ranked.slice(0, 3).map((s, idx) => (
              <div key={s.templateId} className="flex items-center gap-1.5 text-[12px]">
                ...
                <span className="font-bold text-muted-foreground">{s.currentMonthHours}h</span>
              </div>
            ))}
          </div>
        </motion.div>
      ...
      {ranked.length === 0 ? (
        <motion.div
          ...
          className="text-center py-16 px-4 bg-card rounded-[24px] border border-dashed border-gray-300 cursor-pointer hover:border-indigo-400 hover:bg-indigo-50/30 transition-colors group"
        >
          ...
          <h3 className="text-base font-bold text-foreground mb-1 group-hover:text-indigo-900">템플릿이 없습니다</h3>
          <p className="text-sm text-muted-foreground group-hover:text-indigo-600">첫 번째 템플릿을 만들어 활동을 추적해보세요.</p>
        </motion.div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {ranked.map((summary, idx) => {
            ...
            return (
              <motion.div
                ...
                className="bg-card rounded-[24px] p-5 shadow-sm border border-border hover:shadow-lg transition-all group relative overflow-hidden"
              >
                {/* 좌측 색상 스트립 */}
                  ...
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <h4 className="text-[15px] font-bold text-foreground truncate">{summary.title}</h4>
                      ...
                      <p className="text-[11px] font-medium text-muted-foreground truncate mt-0.5">{catNames}</p>
                    </div>
                  </div>
                    ...
                    <AnimatePresence>
                      {openMenuId === summary.templateId && (
                        <motion.div
                          ...
                          className="absolute right-0 top-full mt-1 bg-card rounded-xl shadow-xl border border-border py-1 z-20 min-w-[120px]"
                        >
                          <button onClick={() => handleEdit(summary)} className="w-full flex items-center gap-2 px-3 py-2 text-[13px] font-medium text-foreground hover:bg-muted">
                ...
                <div className="pl-3 mb-3">
                  <div className="flex items-baseline gap-2 mb-1">
                    <span className="text-[11px] font-bold text-muted-foreground">{currentLabel}</span>
                  ...
                  <div className="flex items-baseline gap-2">
                    {summary.customUnitEnabled ? (
                      <>
                        <span className="text-[24px] font-black text-foreground tracking-tighter">{summary.currentMonthUnits}</span>
                        <span className="text-[13px] font-bold text-muted-foreground">차시</span>
                        <span className="text-[13px] font-bold text-muted-foreground/50">/</span>
                        <span className="text-[13px] font-bold text-muted-foreground">{formatMinutesText(summary.currentMonthMinutes)}</span>
                      </>
                    ) : (
                      <>
                        {renderTimePrimary(summary.currentMonthMinutes)}
                        <span className="text-[13px] font-bold text-muted-foreground/50 ml-1">/</span>
                        <span className="text-[13px] font-bold text-muted-foreground ml-1">{summary.currentMonthCount}회</span>
                      </>
                    )}
                  </div>
                  {preset !== 'all' && (
                    <div className="flex items-baseline gap-1.5 mt-2 text-[12px] font-medium text-muted-foreground bg-muted/80 px-2.5 py-1.5 rounded-lg w-fit">
                      <span className="font-bold text-muted-foreground">{prevLabel}:</span>
                      {summary.customUnitEnabled ? (
                        <>
                          <span className="font-bold text-foreground">{summary.prevMonthUnits}</span>
                          <span className="text-[11px]">차시</span>
                          <span className="text-muted-foreground/50">/</span>
                          <span className="font-bold text-foreground">{formatMinutesText(summary.prevMonthMinutes)}</span>
                        </>
                      ) : (
                        <>
                          <span className="font-bold text-foreground">{formatMinutesText(summary.prevMonthMinutes)}</span>
                          <span className="text-muted-foreground/50">/</span>
                          <span className="font-bold text-foreground">{summary.prevMonthCount}</span>
                ...
                <div className="pl-3 space-y-1.5 mb-4">
                  <div className="flex items-center gap-2 text-[12px]">
                    ...
                    <span className="font-bold text-muted-foreground">총 누적:</span>
                    {summary.customUnitEnabled ? (
                      <>
                        <span className="font-black text-foreground">{summary.totalUnits}차시</span>
                        <span className="text-muted-foreground/50">/</span>
                        <span className="font-bold text-foreground">{formatMinutesText(summary.totalMinutes)}</span>
                      </>
                    ) : (
                      <>
                        <span className="font-black text-foreground">{formatMinutesText(summary.totalMinutes)}</span>
                        <span className="text-muted-foreground/50">/</span>
                        <span className="font-bold text-foreground">{summary.totalCount}회</span>
                      </>
                    )}
                  </div>
                  <div className="flex items-center gap-2 text-[12px]">
                    <CalendarDays className="w-3.5 h-3.5 text-muted-foreground" />
                    <span className="font-bold text-muted-foreground">마지막 수행:</span>
                ...
                <button
                  ...
                  className="w-full pl-3 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-muted hover:bg-indigo-50 text-muted-foreground hover:text-indigo-600 text-[13px] font-bold transition-colors border border-border hover:border-indigo-200"
                >
