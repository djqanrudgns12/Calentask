...
import { Trash2, RefreshCcw, Loader2, Database, AlertCircle, X } from 'lucide-react'
...
export function DataHubModal({ open, onOpenChange }: DataHubModalProps) {
  const {
    deletedActivities,
    isLoading,
    handleRestore,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[95vw] sm:max-w-[700px] p-0 overflow-hidden bg-[#f8f9ff] border-none shadow-2xl rounded-2xl flex flex-col">
        <DialogHeader className="px-6 py-4 border-b border-slate-100 bg-white flex flex-row items-center justify-between z-10 shadow-sm relative">
          ...
          <DialogDescription className="sr-only">삭제된 데이터를 복구하거나 영구 삭제합니다.</DialogDescription>
            ...
            <section className="space-y-4">
              <div className="flex flex-col gap-2 p-5 bg-indigo-50 border border-indigo-100 rounded-2xl">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-lg font-bold text-indigo-900">휴지통 및 데이터 관리</h3>
                </div>
            ...
            <section className="space-y-4 flex-1 flex flex-col min-h-0">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-lg font-bold text-slate-800">휴지통</h3>
                </div>
                {deletedActivities.length > 0 && (
                  ...
                  >
                    {isEmptyingTrash ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : null}
                    휴지통 비우기
                  </Button>
                )}
              </div>
