    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent className="w-64 p-2 rounded-xl shadow-lg border-gray-100 bg-white/95 backdrop-blur-md" align="start">
        <div className="px-2 py-1.5 mb-1 border-b border-gray-100">
          <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">내 필터 프리셋</h4>
        </div>
        ...
        <div className="max-h-60 overflow-y-auto py-1 space-y-0.5">
          {isLoading ? (
            ...
            <div className="p-4 text-center text-sm text-gray-400">저장된 프리셋이 없습니다.</div>
          ) : (
            presets.map(preset => (
              <div 
                key={preset.id} 
                className={cn(
                  ...
                  activePresetId === preset.id ? "bg-indigo-50" : "hover:bg-gray-50"
                )}
                ...
                {editingId === preset.id ? (
                  ...
                  <>
                    <div className="flex items-center gap-2 overflow-hidden">
                      <Star className={cn("w-3.5 h-3.5 shrink-0", activePresetId === preset.id ? "fill-indigo-500 text-indigo-500" : "text-gray-300")} />
                      <span className={cn("text-sm truncate", activePresetId === preset.id ? "font-semibold text-indigo-700" : "text-gray-700")}>
                        {preset.name}
                      </span>
                    </div>
                    
                    <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Button size="icon" variant="ghost" className="h-6 w-6 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50" onClick={(e) => handleStartEdit(e, preset)}>
                        <Pencil className="w-3 h-3" />
                      </Button>
                      <Button size="icon" variant="ghost" className="h-6 w-6 text-gray-400 hover:text-red-600 hover:bg-red-50" onClick={(e) => handleDelete(e, preset)}>
