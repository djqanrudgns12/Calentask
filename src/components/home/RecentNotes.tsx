    ...
    <div className="relative bg-white/85 backdrop-blur-xl rounded-3xl border border-white/60 shadow-[0_8px_32px_-8px_rgba(0,0,0,0.08)] overflow-hidden">
      ...
      <div className="px-6 pt-6 pb-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-amber-100 to-orange-100 flex items-center justify-center shadow-inner border border-white/50">
            <NotebookPen className="w-5 h-5 text-amber-600" />
          </div>
          <div>
            <h2 className="text-base font-extrabold text-slate-800 tracking-tight">최근 노트</h2>
      ...
      <div className="px-5 pb-5 space-y-2">
        {recentNotes.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="text-sm font-bold text-slate-400">아직 작성한 노트가 없어요</p>
            <button
              onClick={handleViewAll}
          ...
          recentNotes.map((note) => (
            <motion.button
              ...
              whileHover={{ y: -1 }}
              className="w-full text-left p-3.5 rounded-2xl bg-slate-50/60 hover:bg-white border border-transparent hover:border-slate-100/50 hover:shadow-[0_4px_16px_-6px_rgba(0,0,0,0.06)] transition-all group"
            >
